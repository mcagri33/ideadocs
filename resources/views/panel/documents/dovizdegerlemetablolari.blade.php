...
@section('title', 'Döviz Değerleme Tabloları')
...
@section('content')
  ...
  <div class="col-12">
    <div class="card mb-4">
      ...
      <div class="card-body">
        <form id="uploadForm"  action="{{route('castle.dovizdegerlemetablolari.store')}}" enctype="multipart/form-data" method="post">
          @csrf
          <div class="fallback">
  ...
  <div class="card">
    <div class="table-responsive text-nowrap">
      <table class="table">
        ...
        <?php $count = 1; ?>
        @foreach($dovizDegerlemeTablolari as $ddt)
          <tr>
            <td>{{ $dovizDegerlemeTablolari->perPage() * ($dovizDegerlemeTablolari->currentPage() - 1) + $count }}</td>
            <?php $count++; ?>
            <td>{{ $ddt->document_name }}</td>
            <td>{{ $ddt->document_type }}</td>
            <td>{{ $ddt->created_at }}</td>
            <td>
              @if($ddt->status == 2)
            ...
            <td><a href="{{ route('castle.dovizdegerlemetablolari.download', ['file' => $ddt->id]) }}">Dosyayı İndir</a>
            </td>
          </tr>
        @endforeach
        </tbody>
    ...
    document.getElementById('uploadForm').addEventListener('submit', function(event) {
      ...
      axios.post(form.action, formData, {
        ...
        Swal.fire({
          ...
          window.location.href = "{{ route('castle.dovizdegerlemetablolari.index') }}";
        });
      }).catch(error => {
        progressDiv.style.display = 'none';

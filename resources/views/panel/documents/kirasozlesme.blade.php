...
@section('title', 'Kira Sözleşmeleri')
...
@section('content')
  ...
  <div class="col-12">
    <div class="card mb-4">
      ...
      <div class="card-body">
        <form id="uploadForm"  action="{{route('castle.kirasozlesme.store')}}" enctype="multipart/form-data" method="post">
          @csrf
          <div class="fallback">
  ...
  <div class="card">
    <div class="table-responsive text-nowrap">
      <table class="table">
        ...
        <?php $count = 1; ?>
        @foreach($kiraSozlesme as $ks)
          <tr>
            <td>{{ $kiraSozlesme->perPage() * ($kiraSozlesme->currentPage() - 1) + $count }}</td>
            <?php $count++; ?>
            <td>{{ $ks->document_name }}</td>
            <td>{{ $ks->document_type }}</td>
            <td>{{ $ks->created_at }}</td>
            <td>
              @if($ks->status == 2)
            ...
            <td><a href="{{ route('castle.kirasozlesme.download', ['file' => $ks->id]) }}">Dosyayı İndir</a>
            </td>
          </tr>
        @endforeach
        </tbody>
    ...
    document.getElementById('uploadForm').addEventListener('submit', function(event) {
      ...
      axios.post(form.action, formData, {
        ...
        Swal.fire({
          ...
          window.location.href = "{{ route('castle.kirasozlesme.index') }}";
        });
      }).catch(error => {
        progressDiv.style.display = 'none';

...
@section('title', 'Organizasyon Şeması')


@section('content')
  @can('organizasyon-semasi')
  ...
  <div class="col-12">
    <div class="card mb-4">
      ...
      <div class="card-body">
        <form id="uploadForm"  action="{{route('castle.organizasyonsemasi.store')}}" enctype="multipart/form-data" method="post">
          @csrf
          <div class="fallback">
  ...
  <div class="card">
    <div class="table-responsive text-nowrap">
      <table class="table">
        ...
        <?php $count = 1; ?>
        @foreach($organizasyonSemasi as $os)
          <tr>
            <td>{{ $organizasyonSemasi->perPage() * ($organizasyonSemasi->currentPage() - 1) + $count }}</td>
            <?php $count++; ?>
            <td>{{ $os->document_name }}</td>
            <td>{{ $os->document_type }}</td>
            <td>{{ $os->created_at }}</td>
            <td>
              @if($os->status == 2)
            ...
            <td><a href="{{ route('castle.organizasyonsemasi.download', ['file' => $os->id]) }}">Dosyayı İndir</a>
            </td>
          </tr>
        @endforeach
        </tbody>
    ...
    document.getElementById('uploadForm').addEventListener('submit', function(event) {
      ...
      axios.post(form.action, formData, {
        ...
        Swal.fire({
          ...
          window.location.href = "{{ route('castle.organizasyonsemasi.index') }}";
        });
      }).catch(error => {
        progressDiv.style.display = 'none';

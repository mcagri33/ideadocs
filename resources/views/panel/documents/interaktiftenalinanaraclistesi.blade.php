...
@section('title', 'İnteraktiften Alınan Araç Listesi')


@section('content')
  @can('interaktif-alinan-arac-listesi')
  ...
  <div class="col-12">
    <div class="card mb-4">
      ...
      <div class="card-body">
        <form id="uploadForm"  action="{{route('castle.interaktiftenalinanaraclistesi.store')}}" enctype="multipart/form-data" method="post">
          @csrf
          <div class="fallback">
  ...
  <div class="card">
    <div class="table-responsive text-nowrap">
      <table class="table">
        ...
        <?php $count = 1; ?>
        @foreach($interaktiftenAlinanAracListesi as $ial)
          <tr>
            <td>{{ $interaktiftenAlinanAracListesi->perPage() * ($interaktiftenAlinanAracListesi->currentPage() - 1) + $count }}</td>
            <?php $count++; ?>
            <td>{{ $ial->document_name }}</td>
            <td>{{ $ial->document_type }}</td>
            <td>{{ $ial->created_at }}</td>
            <td>
              @if($ial->status == 2)
            ...
            <td><a href="{{ route('castle.interaktiftenalinanaraclistesi.download', ['file' => $ial->id]) }}">Dosyayı İndir</a>
            </td>
          </tr>
        @endforeach
        </tbody>
      ...
      <div class="d-flex justify-content-center">
        {!! $interaktiftenAlinanAracListesi->links() !!}
      </div>
    </div>
  </div>
    ...
    document.getElementById('uploadForm').addEventListener('submit', function(event) {
      ...
      axios.post(form.action, formData, {
        ...
        Swal.fire({
          ...
          window.location.href = "{{ route('castle.interaktiftenalinanaraclistesi.index') }}";
        });
      }).catch(error => {
        progressDiv.style.display = 'none';

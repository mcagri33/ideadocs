...
@section('title', 'Satış Fatura Listesi')


@section('content')
  @can('satis-fatura-listesi')
  ...
  <div class="col-12">
    <div class="card mb-4">
      ...
      <div class="card-body">
        <form id="uploadForm"  action="{{route('castle.satisfaturalarilistesi.store')}}" enctype="multipart/form-data" method="post">
          @csrf
          <div class="fallback">
  ...
  <div class="card">
    <div class="table-responsive text-nowrap">
      <table class="table">
        ...
        <?php $count = 1; ?>
        @foreach($satisFaturaListesi as $sft)
          <tr>
            <td>{{ $satisFaturaListesi->perPage() * ($satisFaturaListesi->currentPage() - 1) + $count }}</td>
            <?php $count++; ?>
            <td>{{ $sft->document_name }}</td>
            <td>{{ $sft->document_type }}</td>
            ...
            <td><a href="{{ route('castle.satisfaturalarilistesi.download', ['file' => $sft->id]) }}">Dosyayı İndir</a>
            </td>
          </tr>
        @endforeach
        </tbody>
    ...
    document.getElementById('uploadForm').addEventListener('submit', function(event) {
      ...
      axios.post(form.action, formData, {
        ...
        Swal.fire({
          ...
          window.location.href = "{{ route('castle.satisfaturalarilistesi.index') }}";
        });
      }).catch(error => {
        progressDiv.style.display = 'none';

...
@section('title', 'Şüpheli Alacaklar İcra Evrakları')


@section('content')
  @can('supheli-alacaklar-icra-evraklari')
  ...
  <div class="col-12">
    <div class="card mb-4">
      ...
      <div class="card-body">
        <form id="uploadForm"  action="{{route('castle.suphelialacaklar.store')}}" enctype="multipart/form-data" method="post">
          @csrf
          <div class="fallback">
  ...
  <div class="card">
    <div class="table-responsive text-nowrap">
      <table class="table">
        ...
        <?php $count = 1; ?>
        @foreach($supheliAlacaklar as $sal)
          <tr>
            <td>{{ $supheliAlacaklar->perPage() * ($supheliAlacaklar->currentPage() - 1) + $count }}</td>
            <?php $count++; ?>
            <td>{{ $sal->document_name }}</td>
            <td>{{ $sal->document_type }}</td>
            ...
            <td><a href="{{ route('castle.suphelialacaklar.download', ['file' => $sal->id]) }}">Dosyayı İndir</a>
            </td>
          </tr>
        @endforeach
        </tbody>
    ...
    document.getElementById('uploadForm').addEventListener('submit', function(event) {
      ...
      axios.post(form.action, formData, {
        ...
        Swal.fire({
          ...
          window.location.href = "{{ route('castle.suphelialacaklar.index') }}";
        });
      }).catch(error => {
        progressDiv.style.display = 'none';

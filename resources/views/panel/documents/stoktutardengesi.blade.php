...
@section('title', 'Stok Tutar Dengesi')


@section('content')
  @can('stok-tutar-dengesi')
  ...
  <div class="col-12">
    <div class="card mb-4">
      ...
      <div class="card-body">
        <form id="uploadForm"  action="{{route('castle.stoktutardengesi.store')}}" enctype="multipart/form-data" method="post">
          @csrf
          <div class="fallback">
  ...
  <div class="card">
    <div class="table-responsive text-nowrap">
      <table class="table">
        ...
        <?php $count = 1; ?>
        @foreach($stokTutarDengesi as $std)
          <tr>
            <td>{{ $stokTutarDengesi->perPage() * ($stokTutarDengesi->currentPage() - 1) + $count }}</td>
            <?php $count++; ?>
            <td>{{ $std->document_name }}</td>
            <td>{{ $std->document_type }}</td>
            ...
            <td><a href="{{ route('castle.stoktutardengesi.download', ['file' => $std->id]) }}">Dosyayı İndir</a>
            </td>
          </tr>
        @endforeach
        </tbody>
    ...
    document.getElementById('uploadForm').addEventListener('submit', function(event) {
      ...
      axios.post(form.action, formData, {
        ...
        Swal.fire({
          ...
          window.location.href = "{{ route('castle.stoktutardengesi.index') }}";
        });
      }).catch(error => {
        progressDiv.style.display = 'none';

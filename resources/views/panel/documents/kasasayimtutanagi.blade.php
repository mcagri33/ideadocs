...
@section('title', 'Kasa Sayım Tutanağı')


@section('content')
  @can('kasa-sayim-tutanagi')
  ...
  <div class="col-12">
    <div class="card mb-4">
      ...
      <div class="card-body">
        <form id="uploadForm"  action="{{route('castle.kasasayimtutanagi.store')}}" enctype="multipart/form-data" method="post">
          @csrf
          <div class="fallback">
  ...
  <div class="card">
    <div class="table-responsive text-nowrap">
      <table class="table">
        ...
        <?php $count = 1; ?>
        @foreach($kasaSayimTutanagi as $kst)
          <tr>
            <td>{{ $kasaSayimTutanagi->perPage() * ($kasaSayimTutanagi->currentPage() - 1) + $count }}</td>
            <?php $count++; ?>
            <td>{{ $kst->document_name }}</td>
            <td>{{ $kst->document_type }}</td>
            <td>{{ $kst->created_at }}</td>
            <td>
              @if($kst->status == 2)
            ...
            <td><a href="{{ route('castle.kasasayimtutanagi.download', ['file' => $kst->id]) }}">Dosyayı İndir</a>
            </td>
          </tr>
        @endforeach
        </tbody>
    ...
    document.getElementById('uploadForm').addEventListener('submit', function(event) {
      ...
      axios.post(form.action, formData, {
        ...
        Swal.fire({
          ...
          window.location.href = "{{ route('castle.kasasayimtutanagi.index') }}";
        });
      }).catch(error => {
        progressDiv.style.display = 'none';

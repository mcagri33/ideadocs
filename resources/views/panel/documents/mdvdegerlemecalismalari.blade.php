...
@section('title', 'Mdv Değerleme Çalışmaları')


@section('content')
  @can('mdv-degerleme-calismalari')
  ...
  <div class="col-12">
    <div class="card mb-4">
      ...
      <div class="card-body">
        <form id="uploadForm"  action="{{route('castle.mdvdegerlemecalismalari.store')}}" enctype="multipart/form-data" method="post">
          @csrf
          <div class="fallback">
  ...
  <div class="card">
    <div class="table-responsive text-nowrap">
      <table class="table">
        ...
        <?php $count = 1; ?>
        @foreach($mdvDegerlemeCalismalari as $mdc)
          <tr>
            <td>{{ $mdvDegerlemeCalismalari->perPage() * ($mdvDegerlemeCalismalari->currentPage() - 1) + $count }}</td>
            <?php $count++; ?>
            <td>{{ $mdc->document_name }}</td>
            <td>{{ $mdc->document_type }}</td>
            ...
            <td><a href="{{ route('castle.mdvdegerlemecalismalari.download', ['file' => $mdc->id]) }}">Dosyayı İndir</a>
            </td>
          </tr>
        @endforeach
        </tbody>
    ...
    document.getElementById('uploadForm').addEventListener('submit', function(event) {
      ...
      axios.post(form.action, formData, {
        ...
        Swal.fire({
          ...
          window.location.href = "{{ route('castle.mdvdegerlemecalismalari.index') }}";
        });
      }).catch(error => {
        progressDiv.style.display = 'none';

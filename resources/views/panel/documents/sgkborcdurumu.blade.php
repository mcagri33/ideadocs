...
@section('title', 'Sgk Borç Durumunu Gösteren Yazı')
  ...
  @can('sgk-borc-durumu')
    ...
    <div class="col-12">
      <div class="card mb-4">
        ...
        <div class="card-body">
          <form id="uploadForm"  action="{{route('castle.sgkborcdurumu.store')}}" enctype="multipart/form-data" method="post">
            @csrf
            <div class="fallback">
    ...
    <div class="card">
      <div class="table-responsive text-nowrap">
        <table class="table">
          ...
          <?php $count = 1; ?>
          @foreach($sgkBorcDurumu as $sbd)
            <tr>
              <td>{{ $sgkBorcDurumu->perPage() * ($sgkBorcDurumu->currentPage() - 1) + $count }}</td>
              <?php $count++; ?>
              <td>{{ $sbd->document_name }}</td>
              <td>{{ $sbd->document_type }}</td>
              ...
              <td><a href="{{ route('castle.sgkborcdurumu.download', ['file' => $sbd->id]) }}">Dosyayı İndir</a>
              </td>
            </tr>
          @endforeach
          </tbody>
      ...
      document.getElementById('uploadForm').addEventListener('submit', function(event) {
        ...
        axios.post(form.action, formData, {
          ...
          Swal.fire({
            ...
            window.location.href = "{{ route('castle.sgkborcdurumu.index') }}";
          });
        }).catch(error => {
          progressDiv.style.display = 'none';

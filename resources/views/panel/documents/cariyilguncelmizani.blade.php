...
@section('title', 'Cari Yıl Güncel Mizanı')


@section('content')
  @can('cari-yil-guncel-mizani')
  ...
  <div class="col-12">
    <div class="card mb-4">
      ...
      <div class="card-body">
        <form id="uploadForm"  action="{{route('castle.cariyilguncelmizani.store')}}" enctype="multipart/form-data" method="post">
          @csrf
          <div class="fallback">
  ...
  <div class="card">
    <div class="table-responsive text-nowrap">
      <table class="table">
        ...
        <?php $count = 1; ?>
        @foreach($cariYilGuncelMizani as $cygm)
          <tr>
            <td>{{ $cariYilGuncelMizani->perPage() * ($cariYilGuncelMizani->currentPage() - 1) + $count }}</td>
            <?php $count++; ?>
            <td>{{ $cygm->document_name }}</td>
            <td>{{ $cygm->document_type }}</td>
            <td>{{ $cygm->created_at }}</td>
            <td>
              @if($cygm->status == 2)
                <span class="badge bg-label-warning me-1">Onay Bekliyor</span>
              @elseif($cygm->status == 1)
                <span class="badge bg-label-primary me-1">Onaylandı</span>
              @else
                <span class="badge bg-label-danger me-1">Red Edildi</span>
              @endif
            </td>
            <td><a href="{{ route('castle.cariyilguncelmizani.download', ['file' => $cygm->id]) }}">Dosyayı İndir</a>
            </td>
          </tr>
        @endforeach
        </tbody>
    ...
    document.getElementById('uploadForm').addEventListener('submit', function(event) {
      ...
      axios.post(form.action, formData, {
        ...
        Swal.fire({
          ...
          window.location.href = "{{ route('castle.cariyilguncelmizani.index') }}";
        });
      }).catch(error => {
        progressDiv.style.display = 'none';

...
@section('title', 'Satışların Maliyet Çalışması')


@section('content')
  @can('satislarin-maliyet-calismasi')
  ...
  <div class="col-12">
    <div class="card mb-4">
      ...
      <div class="card-body">
        <form id="uploadForm"  action="{{route('castle.satislarinmaliyetcalismasi.store')}}" enctype="multipart/form-data" method="post">
          @csrf
          <div class="fallback">
  ...
  <div class="card">
    <div class="table-responsive text-nowrap">
      <table class="table">
        ...
        <?php $count = 1; ?>
        @foreach($satislarinMaliyetCalismasi as $smc)
          <tr>
            <td>{{ $satislarinMaliyetCalismasi->perPage() * ($satislarinMaliyetCalismasi->currentPage() - 1) + $count }}</td>
            <?php $count++; ?>
            <td>{{ $smc->document_name }}</td>
            <td>{{ $smc->document_type }}</td>
            <td>{{ $smc->created_at }}</td>
            <td>
              @if($smc->status == 2)
            ...
            <td><a href="{{ route('castle.satislarinmaliyetcalismasi.download', ['file' => $smc->id]) }}">Dosyayı İndir</a>
            </td>
          </tr>
        @endforeach
        </tbody>
      ...
      <div class="d-flex justify-content-center">
        {!! $satislarinMaliyetCalismasi->links() !!}
      </div>
    </div>
  </div>
    ...
    document.getElementById('uploadForm').addEventListener('submit', function(event) {
      ...
      axios.post(form.action, formData, {
        ...
        Swal.fire({
          ...
          window.location.href = "{{ route('castle.satislarinmaliyetcalismasi.index') }}";
        });
      }).catch(error => {
        progressDiv.style.display = 'none';

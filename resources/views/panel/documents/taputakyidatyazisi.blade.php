...
@section('title', 'Tapu Takyidat Yazısı')

@section('content')
  @can('tapu-takyidat-yazisi')
    ...
    <div class="col-12">
      <div class="card mb-4">
        ...
        <div class="card-body">
          <form id="uploadForm"  action="{{route('castle.taputakyidatyazisi.store')}}" enctype="multipart/form-data" method="post">
            @csrf
            <div class="fallback">
    ...
    <div class="card">
      <div class="table-responsive text-nowrap">
        <table class="table">
          ...
          <?php $count = 1; ?>
          @foreach($tapuTakyidatYazisi as $tty)
            <tr>
              <td>{{ $tapuTakyidatYazisi->perPage() * ($tapuTakyidatYazisi->currentPage() - 1) + $count }}</td>
              <?php $count++; ?>
              <td>{{ $tty->document_name }}</td>
              <td>{{ $tty->document_type }}</td>
              <td>{{ $tty->created_at }}</td>
              <td>
                @if($tty->status == 2)
              ...
              <td><a href="{{ route('castle.taputakyidatyazisi.download', ['file' => $tty->id]) }}">Dosyayı İndir</a>
              </td>
            </tr>
          @endforeach
          </tbody>
      ...
      document.getElementById('uploadForm').addEventListener('submit', function(event) {
        ...
        axios.post(form.action, formData, {
          ...
          Swal.fire({
            ...
            window.location.href = "{{ route('castle.taputakyidatyazisi.index') }}";
          });
        }).catch(error => {
          progressDiv.style.display = 'none';

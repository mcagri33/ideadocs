...
@section('title', 'Vergi Dairesi Borç Durumunu Gösteren Yazı')

@section('content')
  @can('vergi-dairesi-borc-durumu')
    ...
    <div class="col-12">
      <div class="card mb-4">
        ...
        <div class="card-body">
          <form id="uploadForm"  action="{{route('castle.vergidairesiborc.store')}}" enctype="multipart/form-data" method="post">
            @csrf
            <div class="fallback">
    ...
    <div class="card">
      <div class="table-responsive text-nowrap">
        <table class="table">
          ...
          <?php $count = 1; ?>
          @foreach($vergiDairesiBorcDurumu as $vdbd)
            <tr>
              <td>{{ $vergiDairesiBorcDurumu->perPage() * ($vergiDairesiBorcDurumu->currentPage() - 1) + $count }}</td>
              <?php $count++; ?>
              <td>{{ $vdbd->document_name }}</td>
              <td>{{ $vdbd->document_type }}</td>
              <td>{{ $vdbd->created_at }}</td>
              <td>
                @if($vdbd->status == 2)
                  <span class="badge bg-label-warning me-1">Onay Bekliyor</span>
                @elseif($vdbd->status == 1)
                  <span class="badge bg-label-primary me-1">Onaylandı</span>
                @else
                  <span class="badge bg-label-danger me-1">Red Edildi</span>
                @endif
              </td>
              <td><a href="{{ route('castle.vergidairesiborc.download', ['file' => $vdbd->id]) }}">Dosyayı İndir</a>
              </td>
            </tr>
          @endforeach
          </tbody>
      ...
      document.getElementById('uploadForm').addEventListener('submit', function(event) {
        ...
        axios.post(form.action, formData, {
          ...
          Swal.fire({
            ...
            window.location.href = "{{ route('castle.vergidairesiborc.index') }}";
          });
        }).catch(error => {
          progressDiv.style.display = 'none';

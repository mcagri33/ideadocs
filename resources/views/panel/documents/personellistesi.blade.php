...
@section('content')
  ...
  <div class="col-12">
    <div class="card mb-4">
      ...
      <div class="card-body">
        <form id="uploadForm"  action="{{route('castle.personellistesi.store')}}" enctype="multipart/form-data" method="post">
          @csrf
          <div class="fallback">
  ...
  <div class="card">
    <div class="table-responsive text-nowrap">
      <table class="table">
        ...
        <?php $count = 1; ?>
        @foreach($personelListesi as $pl)
          <tr>
            <td>{{ $personelListesi->perPage() * ($personelListesi->currentPage() - 1) + $count }}</td>
            <?php $count++; ?>
            <td>{{ $pl->document_name }}</td>
            <td>{{ $pl->document_type }}</td>
            <td>{{ $pl->created_at }}</td>
            <td>
              @if($pl->status == 2)
            ...
            <td><a href="{{ route('castle.personellistesi.download', ['file' => $pl->id]) }}">Dosyayı İndir</a>
            </td>
          </tr>
        @endforeach
        </tbody>
    ...
    document.getElementById('uploadForm').addEventListener('submit', function(event) {
      ...
      axios.post(form.action, formData, {
        ...
        Swal.fire({
          ...
          window.location.href = "{{ route('castle.personellistesi.index') }}";
        });
      }).catch(error => {
        progressDiv.style.display = 'none';

...
@section('title', 'Tüm Yıl Muavinleri')
...
@section('content')
  ...
  <div class="col-12">
    <div class="card mb-4">
      ...
      <div class="card-body">
        <form id="uploadForm"  action="{{route('castle.tumyilmuavin.store')}}" enctype="multipart/form-data" method="post">
          @csrf
          <div class="fallback">
  ...
  <div class="card">
    <div class="table-responsive text-nowrap">
      <table class="table">
        ...
        <?php $count = 1; ?>
        @foreach($tumYilMuavin as $tym)
          <tr>
            <td>{{ $tumYilMuavin->perPage() * ($tumYilMuavin->currentPage() - 1) + $count }}</td>
            <?php $count++; ?>
            <td>{{ $tym->document_name }}</td>
            <td>{{ $tym->document_type }}</td>
            <td>{{ $tym->created_at }}</td>
            <td>
              @if($tym->status == 2)
            ...
            <td><a href="{{ route('castle.tumyilmuavin.download', ['file' => $tym->id]) }}">Dosyayı İndir</a>
            </td>
          </tr>
        @endforeach
        </tbody>
    ...
    document.getElementById('uploadForm').addEventListener('submit', function(event) {
      ...
      axios.post(form.action, formData, {
        ...
        Swal.fire({
          ...
          window.location.href = "{{ route('castle.tumyilmuavin.index') }}";
        });
      }).catch(error => {
        progressDiv.style.display = 'none';

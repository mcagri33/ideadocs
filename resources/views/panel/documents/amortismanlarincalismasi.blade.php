...
@section('title', 'Amortisman Çalışması')


@section('content')
  @can('amortisman-calismasi')
  ...
  <div class="col-12">
    <div class="card mb-4">
      ...
      <div class="card-body">
        <form id="uploadForm"  action="{{route('castle.amortismanlarincalismasi.store')}}" enctype="multipart/form-data" method="post">
          @csrf
          <div class="fallback">
  ...
  <div class="card">
    <div class="table-responsive text-nowrap">
      <table class="table">
        ...
        <?php $count = 1; ?>
        @foreach($amortismanCalismasi as $ac)
          <tr>
            <td>{{ $amortismanCalismasi->perPage() * ($amortismanCalismasi->currentPage() - 1) + $count }}</td>
            <?php $count++; ?>
            <td>{{ $ac->document_name }}</td>
            <td>{{ $ac->document_type }}</td>
            <td>{{ $ac->created_at }}</td>
            <td>
              @if($ac->status == 2)
            ...
            <td><a href="{{ route('castle.amortismanlarincalismasi.download', ['file' => $ac->id]) }}">Dosyayı İndir</a>
            </td>
          </tr>
        @endforeach
        </tbody>
    ...
    document.getElementById('uploadForm').addEventListener('submit', function(event) {
      ...
      axios.post(form.action, formData, {
        ...
        Swal.fire({
          ...
          window.location.href = "{{ route('castle.amortismanlarincalismasi.index') }}";
        });
      }).catch(error => {
        progressDiv.style.display = 'none';

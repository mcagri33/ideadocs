...
@section('title', 'Mizan ve Kurumlar Vergisi Beyanamesi')

@section('content')
  @can('mizan-kurumlar-vergisi-beyaname')
    ...
    <div class="col-12">
      <div class="card mb-4">
        ...
        <div class="card-body">
          <form id="uploadForm"  action="{{route('castle.mizankurumlarvergisibeyanamesi.store')}}" enctype="multipart/form-data" method="post">
            @csrf
            <div class="fallback">
    ...
    <div class="card">
      <div class="table-responsive text-nowrap">
        <table class="table">
          ...
          <?php $count = 1; ?>
          @foreach($mizanKurumlarVergisiBeyanamesi as $mkvb)
            <tr>
              <td>{{ $mizanKurumlarVergisiBeyanamesi->perPage() * ($mizanKurumlarVergisiBeyanamesi->currentPage() - 1) + $count }}</td>
              <?php $count++; ?>
              <td>{{ $mkvb->document_name }}</td>
              <td>{{ $mkvb->document_type }}</td>
              <td>{{ $mkvb->created_at }}</td>
              <td>
                @if($mkvb->status == 2)
                  <span class="badge bg-label-warning me-1">Onay Bekliyor</span>
                @elseif($mkvb->status == 1)
                  <span class="badge bg-label-primary me-1">Onaylandı</span>
                @else
                  <span class="badge bg-label-danger me-1">Red Edildi</span>
                @endif
              </td>
              <td><a href="{{ route('castle.mizankurumlarvergisibeyanamesi.download', ['file' => $mkvb->id]) }}">Dosyayı İndir</a>
              </td>
            </tr>
          @endforeach
          </tbody>
        ...
        <div class="d-flex justify-content-center">
          {!! $mizanKurumlarVergisiBeyanamesi->links() !!}
        </div>
      </div>
    </div>
      ...
      document.getElementById('uploadForm').addEventListener('submit', function(event) {
        ...
        axios.post(form.action, formData, {
          ...
          Swal.fire({
            ...
            window.location.href = "{{ route('castle.mizankurumlarvergisibeyanamesi.index') }}";
          });
        }).catch(error => {
          progressDiv.style.display = 'none';

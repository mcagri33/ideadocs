...
@section('title', 'Stok Miktar Dengesi')


@section('content')
  @can('stok-miktar-dengesi')
  ...
  <div class="col-12">
    <div class="card mb-4">
      ...
      <div class="card-body">
        <form id="uploadForm"  action="{{route('castle.stokmiktardengesi.store')}}" enctype="multipart/form-data" method="post">
          @csrf
          <div class="fallback">
  ...
  <div class="card">
    <div class="table-responsive text-nowrap">
      <table class="table">
        ...
        <?php $count = 1; ?>
        @foreach($stokMiktarDengesi as $smd)
          <tr>
            <td>{{ $stokMiktarDengesi->perPage() * ($stokMiktarDengesi->currentPage() - 1) + $count }}</td>
            <?php $count++; ?>
            <td>{{ $smd->document_name }}</td>
            <td>{{ $smd->document_type }}</td>
            <td>{{ $smd->created_at }}</td>
            <td>
              @if($smd->status == 2)
            ...
            <td><a href="{{ route('castle.stokmiktardengesi.download', ['file' => $smd->id]) }}">Dosyayı İndir</a>
            </td>
          </tr>
        @endforeach
        </tbody>
    ...
    document.getElementById('uploadForm').addEventListener('submit', function(event) {
      ...
      axios.post(form.action, formData, {
        ...
        Swal.fire({
          ...
          window.location.href = "{{ route('castle.stokmiktardengesi.index') }}";
        });
      }).catch(error => {
        progressDiv.style.display = 'none';

...
@section('content')
  ...
  <div class="col-12">
    <div class="card mb-4">
      ...
      <div class="card-body">
        <form id="uploadForm"  action="{{route('castle.alinanverilencekler.store')}}" enctype="multipart/form-data" method="post">
          @csrf
          <div class="fallback">
  ...
  <div class="card">
    <div class="table-responsive text-nowrap">
      <table class="table">
        ...
        <?php $count = 1; ?>
        @foreach($alinanverilencekler as $avc)
          <tr>
            <td>{{ $alinanverilencekler->perPage() * ($alinanverilencekler->currentPage() - 1) + $count }}</td>
            <?php $count++; ?>
            <td>{{ $avc->document_name }}</td>
            <td>{{ $avc->document_type }}</td>
            <td>{{ $avc->created_at }}</td>
            <td>
              @if($avc->status == 2)
            ...
            <td><a href="{{ route('castle.alinanverilencekler.download', ['file' => $avc->id]) }}">Dosyayı İndir</a>
            </td>
          </tr>
        @endforeach
        </tbody>
    ...
    document.getElementById('uploadForm').addEventListener('submit', function(event) {
      ...
      axios.post(form.action, formData, {
        ...
        Swal.fire({
          ...
          window.location.href = "{{ route('castle.alinanverilencekler.index') }}";
        });
      }).catch(error => {
        progressDiv.style.display = 'none';

...
@section('title', 'Banka Mutabakatları')
...
@section('content')
  ...
  <div class="col-12">
    <div class="card mb-4">
      ...
      <div class="card-body">
        <form id="uploadForm"  action="{{route('castle.bankamutabakatlari.store')}}" enctype="multipart/form-data" method="post">
          @csrf
          <div class="fallback">
  ...
  <div class="card">
    <div class="table-responsive text-nowrap">
      <table class="table">
        ...
        <?php $count = 1; ?>
        @foreach($bankaMutabakatlari as $bm)
          <tr>
            <td>{{ $bankaMutabakatlari->perPage() * ($bankaMutabakatlari->currentPage() - 1) + $count }}</td>
            <?php $count++; ?>
            <td>{{ $bm->document_name }}</td>
            <td>{{ $bm->document_type }}</td>
            <td>{{ $bm->created_at }}</td>
            <td>
              @if($bm->status == 2)
            ...
            <td><a href="{{ route('castle.bankamutabakatlari.download', ['file' => $bm->id]) }}">Dosyayı İndir</a>
            </td>
          </tr>
        @endforeach
        </tbody>
    ...
    document.getElementById('uploadForm').addEventListener('submit', function(event) {
      ...
      axios.post(form.action, formData, {
        ...
        Swal.fire({
          ...
          window.location.href = "{{ route('castle.bankamutabakatlari.index') }}";
        });
      }).catch(error => {
        progressDiv.style.display = 'none';

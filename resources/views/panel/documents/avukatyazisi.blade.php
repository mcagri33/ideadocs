  ...
  @can('avukat-yazisi')
    ...
    <div class="col-12">
      <div class="card mb-4">
        ...
        <div class="card-body">
          <form id="uploadForm"  action="{{route('castle.avukatyazisi.store')}}" enctype="multipart/form-data" method="post">
            @csrf
            <div class="fallback">
    ...
    <div class="card">
      <div class="table-responsive text-nowrap">
        <table class="table">
          ...
          <?php $count = 1; ?>
          @foreach($avukatYazisi as $ay)
            <tr>
              <td>{{ $avukatYazisi->perPage() * ($avukatYazisi->currentPage() - 1) + $count }}</td>
              <?php $count++; ?>
              <td>{{ $ay->document_name }}</td>
              <td>{{ $ay->document_type }}</td>
              <td>{{ $ay->created_at }}</td>
              <td>
                @if($ay->status == 2)
              ...
              <td><a href="{{ route('castle.avukatyazisi.download', ['file' => $ay->id]) }}">Dosyayı İndir</a>
              </td>
            </tr>
          @endforeach
          </tbody>
      ...
      document.getElementById('uploadForm').addEventListener('submit', function(event) {
        ...
        axios.post(form.action, formData, {
          ...
          Swal.fire({
            ...
            window.location.href = "{{ route('castle.avukatyazisi.index') }}";
          });
        }).catch(error => {
          progressDiv.style.display = 'none';
